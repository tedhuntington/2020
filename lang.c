...
#ifdef Linux
extern struct words cword[100];  //80
...
extern int ex,ex2;
extern int hx1,hy1,hx2,hy2,rx3,ry3,rx4,ry4,curloc,curloc3,newloc;
  ...
  case 1:   /* 2 words*/
	...
	case 461:
	  if (obj[251].loc==curloc||obj[251].loc==-1)
	    {retsen=999; putout("The drug is powdery.");}
	  else retsen=10;
	default:
	  retsen=11;
	  break;
	 ...
	 switch (tmpi)  /*branch on inv item*/
	   ...
	   case 2:
	     for(c=174;c<179;c++)
	       {
	       if (obj[c].loc==-1)
		 { obj[c].loc=curloc;  obj[c].p[0]=hx1-5; obj[c].p[1]=hy1+HMAN;
		 c=179;  retsen=5;}
	       else
		 retsen=6;
	       }  /*end for*/
	   ...
	   case 131:   /*trash-garbage*/
	     ...
	     break;
	   case 180:  /*money*/
	    if (obj[180].loc==-1) {obj[180].loc=curloc; obj[180].p[0]=hx1-5; obj[180].p[1]=hy1+HMAN+2;
					cash=cash-100;retsen=5;}
	    if (obj[181].loc==-1) {obj[181].loc=curloc; obj[181].p[0]=hx1-5; obj[181].p[1]=hy1+HMAN+2;
					cash=cash-75;retsen=5;}
	    if (obj[187].loc==-1) {obj[187].loc=curloc; obj[187].p[0]=hx1-5; obj[187].p[1]=hy1+HMAN+2;
					cash=cash-20;retsen=5;}
	    if (obj[188].loc==-1) {obj[188].loc=curloc; obj[188].p[0]=hx1-5; obj[188].p[1]=hy1+HMAN+2;
					cash=cash-10;retsen=5;}
	    if (obj[189].loc==-1) {obj[189].loc=curloc; obj[189].p[0]=hx1-5; obj[189].p[1]=hy1+HMAN+2;
					cash=cash-75;retsen=5;}
            for(c=190;c<209;c++)
	      {
	      if (obj[c].loc==-1)
		{obj[c].loc=curloc; cash=cash-40;retsen=5;obj[c].p[0]=hx1-5; obj[c].p[1]=hy1+HMAN+2;}
	      }
	    if (obj[209].loc==-1) {obj[209].loc=curloc; obj[209].p[0]=hx1-5; obj[209].p[1]=hy1+HMAN+2;
					cash0=cash0-100;retsen=5;}
	    if (obj[210].loc==-1) {obj[210].loc=curloc; obj[210].p[0]=hx1-5; obj[210].p[1]=hy1+HMAN+2;
					cash0=cash0-75;retsen=5;}
	    if (obj[211].loc==-1) {obj[211].loc=curloc; obj[211].p[0]=hx1-5; obj[211].p[1]=hy1+HMAN+2;
					cash0=cash0-20;retsen=5;}
	    if (obj[212].loc==-1) {obj[212].loc=curloc; obj[212].p[0]=hx1-5; obj[212].p[1]=hy1+HMAN+2;
					cash0=cash0-10;retsen=5;}
	    if (obj[213].loc==-1) {obj[213].loc=curloc; obj[213].p[0]=hx1-5; obj[213].p[1]=hy1+HMAN+2;
					cash0=cash0-75;retsen=5;}
	    for(c=214;c<233;c++)
	      {
	      if (obj[c].loc==-1)
		{obj[c].loc=curloc; cash0=cash0-40;retsen=5;obj[c].p[0]=hx1-5; obj[c].p[1]=hy1+HMAN+2;}
	      }
	   ...
	   default:   /*reg drop*/
	     ...
	     if (obj[tmpi].loc==-1) {obj[tmpi].loc=curloc;
			  obj[tmpi].p[0]=hx1-5; obj[tmpi].p[1]=hy1+HMAN+2;
			  retsen=5;}
	...
	if (tmpi>1)
	  ...
	  switch(tmpi)
		...
		if (close_enough(141))
	      ...
	      if (curloc==2)
		{
		if (close_enough(2)||(hx1<195&&hx1>110&&hy1>165))
		  {if (obj[179].loc==2) retsen=72;
		  else
		  {obj[179].loc=2; retsen=5;
		    ...
		    if(hy1>165&&hy1<191&&hx1>110&&hx1<195) hy1=hy2=ry3=ry4=165;
		    }
		  }
		else
		  retsen=6;
	...
	break;
	 ...
	 case 4:
		...
		if (close_enough(141))
	      ...
	      if (curloc==2)
		{
		if (close_enough(2)||(hx1<195&&hx1>110&&hy1>150))
...
give()
{
	...
	sent[1]==413||sent[1]==412||sent[1]==489||sent[1]==459||sent[1]==460)
	  ...
	  if (curloc==66&&obj[184].loc==-1) {retsen=999; wdflag=5;
	    putout("Lee takes the music of the USA disk and thanks you.");
	...
	sent[1]==493||sent[1]==413||sent[1]==412||sent[1]==459||sent[1]==460)
	  ...
	  if (curloc==68&&obj[186].loc==-1) {retsen=999;
	    putout("Professor Godless takes the History of Earth disk and thanks you more than one time.");
           retsen=999;
...
if (curloc3==73&&spec[24]==5)
  {
  putout("Professor Sagan says:  'You have done an excellent job.  You have secured the robot here on the moon.  In room C3 there is a shuttle that will take you to a ship the General is in.");
  putout("Take this transquilizer drug and put it in his water supply.  The drug will make the General easy to capture and store in a jail until dead.");
...
if (curloc==80)
  ...
  if (obj[251].loc==-1)
    {
    retsen=999;
    putout("You put the tranquilizer drug in the water supply for the General. The General human is captured. The General human had killed other humans by hand and will be in a jail until dead.");
    putout("You then take the shuttle back to Moon Port 3. You are most popular and rewarded with millions of earth dollars!");
    endcred();
    }
  }
else
...
putout("When I first saw DOOM, I really thought that this game should be 3D.");
#ifdef Linux
putout("This game is running on PC UNIX (Linux)!  I like Linux and think that the free copy of all data will make life more fair and less violent.  Thank you for supporting Linux.");
#endif
putout("What 2020 does have are some pretty interesting, intelligent ideas.");
putout("I think it has a more realistic vision of our future than most stories.");
putout("Just as importantly, 2020 helps people understand that all words are simply sounds or images and no human should be locked in jail for sounds made or images owned.");
...
putout("I strongly urge you to support space exploration (to other suns), the development of the moon and mars. I ask you to stop the locking in jail of humans that have not caused damage, in particular for drugs and prostitution.");
putout("It is also very important to vote. Vote on all government decisions. A 20 year old machine can hear and see thought, a machine still kept secret from most humans.");
putout("Vote out the current laws stopping the owning and use of drugs, the sale of touch and sex, any and all pictures or information about anything, and suicide.");
  ...
  if (close_enough(239)) {  if (spec[25]==1&&obj[28].loc==-1) {
    ...
    if (close_enough(238))
      {		if (spec[25]==1&&obj[28].loc==-1) {
		...
		putout("Man:  Bueno.  ­Muchas gracias!");
		putout("Translator: Good. Thank you very much!");
...
switch(curloc)
  ...
  case 2:  putout("You can sleep to use time.  The robot can help collect the stock.  Open the desk drawer."); break;
  case 4:  putout("You can shower to use 10 minutes of time."); break;
  ...
  case 8:  putout("To get to the other side of the street wait in front of the door for a part between cars to be enough time to get past."); break;
  case 12:
  case 13:
  case 14:
  case 15:
  ...
  case 44:
    ...
    break;
  case 46:
    putout("Get a job from the clerk, if you get what I am saying.");
    break;
  case 47:
  ...
  case 48:
    ...
    break;
  case 57:
    putout("Buy a ticket from one of the clerks. Make sure the translator is on.");
    break;
  case 58:
  case 59:
    putout("Pick a door.");
  ...
  case 65:
    ...
    break;
  case 66:
    putout("Give the music of the usa disk to Lee. damn!");
    break;
  case 68:
    putout("Give the History of the Earth disk to Professor Godless. I had to substitute an empty room because there is an error in the original picture, so yes Professor Godless is not there right now, but 'give cd' works.");
    break;
  case 70:
  ...
  case 73:
    ...
    break;
  case 76:
    putout("Pick the correct door.  I am not telling you what door. You and I have gone too far together to turn back now.");
    break;
  case 80:
    putout("You need to put the drug in the water system for the General, so when the General drinks his coffee (every 10 minutes), the General will be made not conscious and ok to capture.");
    break;
  default:
#ifdef Linux
      putout("No hint needed here humana.");  break;
#endif
#ifdef DOS
