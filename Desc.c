...
strcpy(des[67],"You are in room A3.");
...
strcpy(des[TVC+22],"Science channel:              The military maintains an Ocean Station off the Southern California coast. It's exact location is unknown.");
...
strcpy(des[TVC+68],"Moon channel:                 ????");
strcpy(des[TVC+69],"Public Access channel:        Right to Body Show.");
/*strcpy(des[TVC+67],"Public news channel:          Elected humans in the United Nations ready to contain humans damaging in the USA.");*/
}
...
strcat(t0," killed he implemented some radical changes for the country, including the release of all nonviolent");
spec[6]=0;
putout(t0);
strcpy(t0,"consentual criminals and implementation of the right to body. Other");
strcat(t0," significant changes included the creation of an earth currency, an earth law,");
strcat(t0," the exploration of nearby suns, and the colonization of planet mars and the moons of earth and mars.");
putout(t0);
strcpy(t0,"At the time of his death President Huntington was in possesion");
strcat(t0," of an extremely intelligent, highly dexterous robot.  You are the child of one of President Huntington's cabinet members");
strcat(t0," and are in serious danger. You must avoid the general's army, find the earth's");
putout(t0);
strcpy(t0,"intellectual leaders and secure the dead president's invention.");
putout(t0);
}
...
strcat(t0,"middle key Stops human    ");
